...
class ReactiveCollection(async: AsyncCollection) {
  ...
  def unlock(
    ...
    CoreCommonConvertersScala2.convert(kvOps.unlockReactive(convert(options), id, cas)).map(_ => ())
  }
  ...
  def scan(scanType: ScanType, opts: ScanOptions): SFlux[ScanResult] = {
    FutureConversions.javaFluxToScalaFlux(async.scanRequest(scanType, opts))
  }
}
