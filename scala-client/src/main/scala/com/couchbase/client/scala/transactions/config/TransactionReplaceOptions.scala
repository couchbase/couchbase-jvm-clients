...
/** Operations controlling a transactional replace.
  */
@Stability.Volatile
case class TransactionReplaceOptions private (transcoder: Option[Transcoder] = None) {
  ...
  def transcoder(transcoder: Transcoder): TransactionReplaceOptions = {
    copy(transcoder = Some(transcoder))
  }
}

object TransactionReplaceOptions {
  val Default: TransactionReplaceOptions = TransactionReplaceOptions()
}
