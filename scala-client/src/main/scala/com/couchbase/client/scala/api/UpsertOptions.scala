...
case class UpsertOptions(timeout: FiniteDuration = null,
  ...
  def replicateTo(replicateTo: ReplicateTo.Value): UpsertOptions = copy(replicateTo = replicateTo)

  def persistTo(persistTo: PersistTo.Value): UpsertOptions = copy(persistTo = persistTo)
}


object UpsertOptions {
  def apply(): UpsertOptions = new UpsertOptions()
}
