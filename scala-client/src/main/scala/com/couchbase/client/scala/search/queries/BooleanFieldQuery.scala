...
/** An FTS query that queries fields explicitly indexed as boolean.
  ...
  * @since 1.0.0
  */
case class BooleanFieldQuery(
    private[scala] val value: Boolean,
  ...
  /** If specified, only this field will be matched.
    ...
    */
  def field(field: String): BooleanFieldQuery = {
  ...
  /** The boost parameter is used to increase the relative weight of a clause (with a boost greater than 1) or decrease
    ...
    * @return a copy of this, for chaining
    */
  def boost(boost: Double): BooleanFieldQuery = {
    copy(boost = Some(boost))
  }

  override private[scala] def toCore =
    new CoreBooleanFieldQuery(
      value,
      field.orNull,
