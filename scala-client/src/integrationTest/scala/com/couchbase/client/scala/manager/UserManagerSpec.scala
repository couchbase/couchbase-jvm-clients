...
class UserManagerSpec extends ScalaIntegrationTest {
  ...
  private def assertUserAbsent(username: String): Unit = {
    ...
    assertThrows(classOf[UserNotFoundException], () => users.getUser(username, AuthDomain.Local).block())
  }
  ...
  @Test
  def parseRole(): Unit = {
    val role = read[RoleAndOrigins]("""{"role":"bucket_full_access","bucket_name":"*","origins":[{"type":"user"}]}""")
  ...
  def parseUserAndMetadata3(): Unit = {
    val raw = """{"id":"integration-test-user","domain":"local","roles":[{"role":"ro_admin","origins":[{"type":"group","name":"group-a"}]}],"groups":["group-a"],"external_groups":[],"name":"","password_change_date":"2019-08-16T17:26:31.000Z"}"""
    val user = read[UserAndMetadata](raw)
    assert(user.username == "integration-test-user")
  }
  ...
  def createWithBadRole(): Unit = {
    assertThrows(classOf[CouchbaseException],
      () => {
  ...
  def dropAbsentUser(): Unit = {
    ...
    val e: UserNotFoundException = assertThrows(classOf[UserNotFoundException], () => users.dropUser(name).block())
