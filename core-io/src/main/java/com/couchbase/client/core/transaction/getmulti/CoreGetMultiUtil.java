...
public class CoreGetMultiUtil {
    ...
    public static List<CoreTransactionGetMultiSpec> toSpecs(List<CoreTransactionGetMultiResult> input) {
        return input.stream().map(v -> v.spec).collect(Collectors.toList());
    }
}
