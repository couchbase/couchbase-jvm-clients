 ...
 *
 * <p>It is built on top of a very fast, bounded MPSC (multi-producer single-consumer) queue which allows
 * to quickly send events into the bus without blocking the sender.</p>
 *
 * <p>Subscribers of this API are considered to be non-blocking and if they have to blocking tasks need to
 * fan them out into their own thread pool.</p>
 *
 * <p>Keep in mind to properly {@link #start()} and {@link #stop()} since it runs in its own thread!</p>
 */
public class DefaultEventBus implements EventBus {
    ...
    private DefaultEventBus(final Builder builder) {
        ...
        eventQueue = QueueFactory.newQueue(ConcurrentQueueSpec.createBoundedMpsc(builder.queueCapacity));
