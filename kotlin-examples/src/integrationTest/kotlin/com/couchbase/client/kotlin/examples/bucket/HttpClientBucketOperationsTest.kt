...
class HttpClientBucketOperationsTest {

    @Test
    @Order(1)
    fun `create bucket with http client`() {
    ...
    @Test
    @Order(2)
    fun `update bucket with http client`() {
        ConnectionUtils.withHttpClient { cluster, httpClient ->
            httpClient.post(
                target = HttpTarget.manager(),
    ...
    @Test
    @Order(3)
    fun `get bucket info with http client`() {
    ...
    @Test
    @Order(4)
    fun `get all buckets info with http client`() {
        ConnectionUtils.withHttpClient { _, httpClient ->
            val response = httpClient.get(
                target = HttpTarget.manager(),
                path = "/pools/default/buckets"
    ...
    @Test
    @Order(5)
    fun `get bucket statistics with http client`() {
        ConnectionUtils.withHttpClient { _, httpClient ->
            val response = httpClient.get(
                target = HttpTarget.manager(),
    ...
    @Test
    @Order(6)
    fun `flush bucket with http client`() {
        ConnectionUtils.withHttpClient { _, httpClient ->
            httpClient.post(
                target = HttpTarget.manager(),
    ...
    @Test
    @Order(7)
    fun `delete bucket with http client`() {
        ConnectionUtils.withHttpClient { cluster, httpClient ->
            httpClient.delete(
                target = HttpTarget.manager(),
                path = formatPath("/pools/default/buckets/{}", BUCKET_NAME)
