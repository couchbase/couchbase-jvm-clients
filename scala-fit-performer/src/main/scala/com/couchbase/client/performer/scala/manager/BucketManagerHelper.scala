...
object BucketManagerHelper {
  ...
  def handleBucketManager(cluster: Cluster, command: Command): Result.Builder = {
    ...
    if (bm.hasGetBucket) {
      ...
      response match {
        case Success(bucketSettings: com.couchbase.client.scala.manager.bucket.BucketSettings) =>
          populateResult(result, bucketSettings)
  ...
  def populateResult(
    ...
    response.ejectionMethod match {
      ...
      case EjectionMethod.NotRecentlyUsed =>
        builder.setEvictionPolicy(EvictionPolicyType.NOT_RECENTLY_USED)
