  ...
  private def createOptions(
    ...
    if (hasOptions) {
        ...
        if (opts.getScanConsistency == ScanConsistency.NOT_BOUNDED) {
          ...
          out = out.scanConsistency(
            QueryScanConsistency.RequestPlus(
              if (opts.hasScanWaitMillis)
                Some(Duration(opts.getScanWaitMillis, TimeUnit.MILLISECONDS))
              else None
            )
          )
      ...
      if (opts.getParametersPositionalCount > 0) {
        out = out.parameters(
          QueryParameters.Positional(opts.getParametersPositionalList.asScala.toList: _*)
        )
      }
