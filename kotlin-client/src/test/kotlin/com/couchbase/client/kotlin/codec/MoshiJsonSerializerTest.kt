...
internal class MoshiJsonSerializerTest {
    private val moshi = Moshi.Builder().add(KotlinJsonAdapterFactory()).build()
    private val serializer = MoshiJsonSerializer(moshi)

    @Test
    fun `can customize Moshi JsonAdapter`() {
        // by default, null map values are omitted
        assertArrayEquals("{}".toByteArray(), serializer.serialize(mapOf("foo" to null)))

        // but we should be able to customize the JsonAdapter to serialize nulls
        val customSerializer = MoshiJsonSerializer(moshi) { serializeNulls() }
        assertArrayEquals("{\"foo\":null}".toByteArray(), customSerializer.serialize(mapOf("foo" to null)))
    }
    ...
    fun `list of primitives survives round trip`() {
        val testSubject = listOf("a", 1.0, true)

        val encoded = serializer.serialize(testSubject)
        assertEquals("[\"a\",1.0,true]", encoded.toStringUtf8())
    ...
    data class SerializeMeDifferently(@Json(name = "x") val magicWord: String)

    @Test
    fun `respects Json annotation`() {
