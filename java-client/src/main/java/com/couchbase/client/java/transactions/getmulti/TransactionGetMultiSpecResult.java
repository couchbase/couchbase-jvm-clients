...
public class TransactionGetMultiSpecResult {
    ...
    @Stability.Internal
    public TransactionGetMultiSpecResult(TransactionGetMultiSpec spec,
                                         Optional<CoreTransactionGetResult> r,
                                         JsonSerializer serializer,
                                         @Nullable Transcoder transcoder) {
        this.internal = requireNonNull(r).map(v -> new TransactionGetResult(v, requireNonNull(serializer), transcoder));
    ...
    public TransactionGetResult get() {
        return internal.orElseThrow(() ->
            new CouchbaseException("Called get() but document " + spec + " is not present!"));
    }
