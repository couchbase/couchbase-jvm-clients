...
# USAGE: Run this script from the `tracing-opentelemetry` directory after the JAR is built.
#
# The script exits with a non-zero status code if the tracing-opentelemetry JAR
# contains files from a library that was not shaded correctly.

set -e

# Find the JAR without a "sources" or "javadoc" classifier
JAR_FILE=$(ls target/tracing-opentelemetry-*.jar | grep --invert-match -e "sources" -e "javadoc")
